...
import { FishermanReportView } from "@/components/FishermanReport";
import { ResultTabs } from "@/components/ResultTabs";
import { buildFishermanNarrative, buildPrimaryReason } from "@/lib/reason";
import {
  decisionLabels,
  decisionShortLabels,
  factorBasisLabels,
...
function windowSpeciesLine(result: DecisionResponse, window: { start: string; end: string }): string | null {
...
function WindDirectionBadge({ directionDeg }: { directionDeg: number | null }) {
  if (directionDeg === null) return <span className="hour-wind-dir">ريح غير معروفة</span>;
  return (
    <span
      className="hour-wind-dir"
      title={`الريح قادمة من ${windSectorName(directionDeg)}`}
    >
      <ArrowUp size={12} style={{ transform: `rotate(${directionDeg}deg)` }} aria-hidden="true" />
      <bdi>{windSectorName(directionDeg)}</bdi>
    </span>
  );
}
...
function HourCard({ hour, selected, onSelect }: { hour: HourDecision; selected: boolean; onSelect: () => void }) {
  return (
    ...
    >
      ...
      <WindDirectionBadge directionDeg={hour.forecast.wind_direction_deg} />
...
export function DecisionResult({ result, request, stale = false }: DecisionResultProps) {
  ...
  const narrative = useMemo(
    () => buildFishermanNarrative(result, request),
    [result, request],
  );
  const tone = toneByDecision[result.decision];
  const reasonText = buildPrimaryReason(result);
  const copy = {
  ...
  // بطاقة ميدانية مكتفية أعلى النتيجة: الرياح والموج والرصاص من ساعة النافذة
  // نفسها، حتى يقرأ الصيّاد ويمشي دون فتح أي تبويب.
  const fieldCard = useMemo(() => {
    const hour = fieldContext;
    const lead = result.gear_recommendation?.scenarios[0];
    const items: { label: string; value: string; note?: string; icon: React.ReactNode }[] = [];
    if (hour) {
      items.push({
        icon: <Wind size={16} />,
        label: "الرياح",
        value: `${numberOrDash(hour.forecast.wind_speed_kmh)} كم/س`,
        note:
          hour.forecast.wind_direction_deg === null
            ? undefined
            : `${windSectorName(hour.forecast.wind_direction_deg)} · هبات ${numberOrDash(hour.forecast.wind_gust_kmh)}`,
      });
      items.push({
        icon: <Waves size={16} />,
        label: "الموج",
        value: `${numberOrDash(hour.forecast.wave_height_m, 2)} م`,
        note: `فترة ${numberOrDash(hour.forecast.wave_period_s, 1)} ث`,
      });
    }
    if (lead) {
      items.push({
        icon: <Anchor size={16} />,
        label: "الرصاص",
        value: lead.weight_band_ar,
        note: lead.shape_ar,
      });
    }
    return items;
  }, [fieldContext, result.gear_recommendation]);
  ...
  const share = async () => {
    const text = [
      ...
      `سبب القرار: ${reasonText}`,
      `بوابة المحرك: ${decisionShortLabels[safetyAxis]} · قابلية التنفيذ المادي: ${field.score}/100 (${fieldFeasibilityLabels[field.status]}، ثقة الفحص ${field.confidence}/100)`,
    ...
    <section className={`results-shell result-${tone}`} id="decision" aria-labelledby="decision-title" aria-live="polite">
      ...
      <header className={`decision-hero decision-${result.decision}`}>
        <div className="decision-copy">
          ...
          {result.decision === "go" && cautionFactors.length > 0 && (
            <div className="lead-cautions" role="note">
              <TriangleAlert size={15} />
              <span><strong>نقاط الحذر داخل النافذة:</strong> {cautionFactors.slice(0, 3).map((factor) => factor.explanation_ar).join(" · ")}</span>
            </div>
          )}
        </div>
        ...
        <div className="decision-scores" aria-label="ملخص النقاط">
          ...
          <div className="confidence-card">
            ...
            {result.confidence.breakdown_ar[0] && <p className="confidence-cause">{result.confidence.breakdown_ar[0]}</p>}
          </div>
        </div>
        <div className="not-probability"><Info size={15} /> مؤشر الفرصة النسبي ترتيب مقارن، وليس احتمال مصيد.</div>
      </header>

      {/* بطاقة ميدانية مكتفية: الرياح والموج والرصاص دون فتح أي قسم. */}
      {fieldCard.length > 0 && (
        <div className="field-brief" data-testid="field-brief" aria-label="ملخص ميداني سريع">
          {fieldCard.map((item) => (
            <span key={item.label}>
              {item.icon}
      ...
      <section className="fisherman-narrative" aria-labelledby="fisherman-narrative-title">
        <div className="result-section-head">
          <div><span className="section-icon"><Fish size={18} /></span><span><h3 id="fisherman-narrative-title">قراءة الصيّاد</h3><p>علاش نزيدو أو لا نزيدوش — سرد حتمي من نفس أرقام المحرك.</p></span></div>
        </div>
        <div className="fisherman-paragraphs">
          {narrative.map((paragraph, index) => (
            <p key={paragraph} className={index === 0 ? "fisherman-lead" : undefined}>{paragraph}</p>
          ))}
        </div>
      </section>

      <div className="result-actions" aria-live="polite">
        <button id="spot-report-trigger" className="report-trigger" type="button" aria-expanded={reportVisible} aria-controls="spot-report" onClick={toggleReport}><FileText size={17} />{reportVisible ? "أخفِ التقرير" : "التقرير الكامل (نسخة علمية)"}</button>
        ...
        tabs={[
          {
            ...
            content: (
              ...
              <section className="timeline-section" aria-labelledby="timeline-title">
                <div className="result-section-head">
                  <div><span className="section-icon"><Clock3 size={18} /></span><span><h3 id="timeline-title">القرار ساعة بساعة</h3><p>اختار أي ساعة باش تشوف قيم التوقع ومشتقاته وأسبابه واتجاه الريح.</p></span></div>
                    ...
                    {result.recommended_windows.map((window, index) => {
                      ...
                      return (
                        <article className={index === 0 ? "window-card lead" : "window-card"} key={`${window.start}-${window.end}`}>
                          ...
                          <details className="window-score-details">
                            <summary><ChevronDown size={14} /> درجات النافذة</summary>
                          ...
                          {speciesLine && <small className="window-species"><Sparkles size={13} /> {speciesLine}</small>}
                        </article>
                      );
                    })}
                  </div>
              ...
              {selected && (
                <section className="conditions-section" aria-labelledby="conditions-title">
                  <div className="result-section-head">
                    <div><span className="section-icon"><Compass size={18} /></span><span><h3 id="conditions-title">تفاصيل الساعة · {formatHour(selected.time)}</h3><p>قيم API المتوقعة ومشتقاتها — وليست محطة رصد داخل البقعة.</p></span></div>
                  ...
                  <div className="conditions-grid">
                    ...
                    <Metric
                      ...
                      label="اتجاه الريح"
                      value={selected.forecast.wind_direction_deg === null ? "غير معروف" : windSectorName(selected.forecast.wind_direction_deg)}
                      note={selected.forecast.wind_direction_deg === null ? "اتجاه القدوم ناقص" : `قادمة من ${selected.forecast.wind_direction_deg.toFixed(0)}° · ${windRelationLabel(selected.derived.wind_relation)}`}
                    />
                  ...
                  <details className="hour-diagnostics">
                    <summary><span><Database size={16} /> مشتقات تشخيصية للمهتمين</span><ChevronLeft size={16} /></summary>
                    <div className="conditions-grid">
                  ...
                  <details className="hour-reasons">
            ...
            content: (
              ...
              <section className={`field-feasibility-panel field-${fieldTone[field.status]}`} id="field-feasibility" data-testid="field-feasibility" aria-labelledby="field-feasibility-title">
                <div className="field-panel-head">
                  ...
                  <div className="field-panel-score">
                    ...
                    <em>ثقة الفحص {field.confidence}/100</em>
                  </div>
                </div>
              ...
              {waterContext && (
                <section className={`observation-panel satellite-panel satellite-${waterContext.availability}`} data-testid="coastal-water-context" aria-labelledby="coastal-water-title">
                  ...
                  <div className="observation-boundary satellite-boundary" role="note"><Info size={16} /><span><strong>سياق فقط:</strong> العينة ثابتة على بعد 1 كم باتجاه البحر؛ ليست قياساً ميدانياً ولا توقعاً للموعد. FNU لا تتحول إلى NTU، ولا تغيّر TUR/SPM/CHL/SST القرار أو السلامة أو التنفيذ أو الفرصة أو الثقة؛ CHL لا تشخّص ازدهاراً ضاراً أو نشاط السمك. قراءة TUR الساتلية قناة منفصلة عن سلم القرائن التشغيلية الخمس ولا تُحسب ضمنه كعائلة قرائن للعكارة. ازدهار عضوي كثيف (CHL ≥ 10) مع نقل بحري مرتفع يرفع سلم أدلة الصوفة إلى الإثبات القاهر.</span></div>
                </section>
              )}
              </>
            ),
          ...
          {
            id: "gear",
            label: "العتاد والتكتيك",
            icon: <Anchor size={16} />,
            content: (
              <>
              {result.gear_recommendation && (
                <section className="gear-section" aria-labelledby="gear-title">
                  <div className="result-section-head">
                    <div><span className="section-icon"><Anchor size={18} /></span><span><h3 id="gear-title">العتاد المقترح</h3><p>مجالات وزن وشكل من كتالوج عام؛ ليست أرقاماً دقيقة بلا بيانات عتادك وقاعك.</p></span></div>
                  </div>
                  <GearPanel gear={result.gear_recommendation} heading={false} />
                </section>
              )}

              {result.fisherman_report && <FishermanReportView report={result.fisherman_report} />}
              </>
            ),
          },
          {
            ...
            content: (
              <>
              <details className="tech-details" id="tech-details">
                <summary><span><Database size={18} /> الأرقام الكاملة والتفاصيل الفنية</span><span>المحاور · ميزان العوامل · الفترات · المصادر</span><ChevronLeft size={18} /></summary>
                <div className="tech-details-body">
                  <section className="decision-axes-panel" aria-labelledby="decision-axes-title">
                    ...
                    <div className="decision-axes-grid">
                      ...
                      <article className={`axis-card field-${fieldTone[field.status]}`}>
                        <span><Anchor size={18} /> 02 · قابلية التنفيذ المادي</span>
                        <strong><bdi>{field.score}/100</bdi></strong>
                        <small>{fieldFeasibilityLabels[field.status]} · ثقة الفحص {field.confidence}/100</small>
                      </article>
                  ...
                  <div className="transparency-grid">
                    ...
                    <details className="details-panel">
                      <summary><span><Shield size={19} /><strong>علاش ثقة المدخلات {result.confidence.score}/100؟</strong></span><span>تفكيك المكونات</span><ChevronLeft size={18} /></summary>
                      <div className="details-content">
                        <ul>{result.confidence.breakdown_ar.map((item) => <li key={item}>{item}</li>)}</ul>
                        <ul>{result.confidence.reasons_ar.map((item) => <li key={item}>{item}</li>)}</ul>
                      </div>
                    </details>
              ...
              <GeminiReport result={result} request={request} />
              </>
            ),
          },
        ]}
