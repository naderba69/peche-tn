...
class DecisionResponse(StrictModel):
    schema_version: Literal["3.13"] = SCHEMA_VERSION
    engine_version: str
...
class OpenMeteoClientPayload(StrictModel):
    """Raw Open-Meteo marine/weather responses fetched by the user's browser.

    The browser fetches Open-Meteo directly (CORS) so the request leaves the
    user's own IP address, avoiding rate limits on the server's shared egress
    IP. The engine never trusts this blindly: it re-parses and re-validates
    structure and times exactly like a server-side fetch, and the server keeps
    its own fetch as fallback when this payload is absent or malformed.
    """
...
class ForecastDecisionRequest(StrictModel):
    ...
    test_cast: TestCast | None = None
    official_warning: OfficialWarning | None = None
    open_meteo_data: OpenMeteoClientPayload | None = None
