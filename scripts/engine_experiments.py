...
def check(name: str, ok: bool, detail: str) -> None:
    REPORT.append(
        {"type": "check", "name": name, "status": "PASS" if ok else "FAIL", "detail": detail}
    )
    if not ok:
...
def section1() -> None:
    ...
    check(
        ...
        same,
        "double evaluation compared field by field",
    )

    r0 = evaluate()
    check(
        "baseline golden case is go/safe_window",
        r0.decision == DecisionLevel.GO
        and r0.decision_reason_code == DecisionReasonCode.SAFE_WINDOW,
...
def assert_single_no_go_transition(
    name: str, rows, no_go_threshold: float, threshold_label: str
) -> None:
...
def section2() -> None:
    th_inter_sandy = safety_thresholds(build_dataset().spot, build_dataset().angler)
    ...
    assert_single_no_go_transition(
        ...
        rows,
        th_inter_sandy.sustained_wind_no_go_kmh,
...
def section3() -> None:
    ...
    r = evaluate(per_hour=strong_waves, count=96, start_at=datetime(2026, 9, 4, 0, tzinfo=TZ))
    check(
    ...
    r1 = with_reports(1)
    check(
        "single user fouling report -> caution only (go)",
    ...
    r2 = with_reports(2)
    check(
        "two consistent user reports -> no_go fouling_confirmed",
    ...
    check(
        "one official fouling report -> no_go fouling_confirmed",
    ...
    ds = ds.model_copy(
        update={
            ...
            "test_cast": TestCast(
                cast_at=datetime(2026, 9, 7, 0, tzinfo=TZ) - timedelta(hours=1), hooked=False
            ),
        }
    )
    r4 = ENGINE.evaluate(ds)
    check(
        "clean test cast downgrades a single report back to caution",
    ...
    warn = OfficialWarning(
        kind="strong_wind",
        level="warning",
        issued_at=datetime(2026, 9, 7, 0, tzinfo=TZ),
        text_ar="نشرة رسمية",
    )
    ...
    r6 = ENGINE.evaluate(
        build_dataset(hour_updates={"wind_speed_kmh": 60.0}).model_copy(
            update={"official_warning": warn}
        )
    )
    check(
    ...
    obs = WeatherStationObservation(
        ...
        station_longitude=10.3,
        observed_at=datetime(2026, 9, 7, 0, 0, tzinfo=TZ),
        retrieved_at=datetime(2026, 9, 7, 0, 5, tzinfo=TZ),
        distance_to_spot_km=12.0,
        raw_report="METAR DTTX 0000Z 24012KT 9999 TS",
    )
    ...
    check(
        ...
        r9.decision == DecisionLevel.GO and any(h.safety == DecisionLevel.NO_GO for h in r9.hourly),
...
def section5() -> None:
    ...
    for _ in range(N):
        ...
        ds = build_dataset(
            ...
            spot_updates={
                "shore_type": shore,
                "exposure": rng.choice(["open", "sheltered"]),
            ...
            ds = ds.model_copy(
                update={
                    "official_warning": OfficialWarning(
                        kind=rng.choice(["strong_wind", "rough_sea"]),
                        level="warning",
                        issued_at=datetime(2026, 9, 7, 0, tzinfo=TZ),
                        text_ar="x",
                    )
                ...
                update={
                    "field_reports": [
                        FieldReport(
                            kind="fouling",
                            severity="dense",
                            source=rng.choice(["user", "community"]),
                            reported_at=datetime(2026, 9, 7, 0, tzinfo=TZ)
                            - timedelta(hours=rng.choice([1, 6])),
                        )
                        for _ in range(n)
                    ]
                }
        ...
        if r.decision == DecisionLevel.GO:
            ...
            if (
                ds.official_warning is not None
                and r.decision_reason_code != DecisionReasonCode.OFFICIAL_WARNING
            ):
                violations.append("official warning present but reason is not official_warning")
            if (
                ds.official_warning is None
                and r.decision_reason_code == DecisionReasonCode.OFFICIAL_WARNING
            ):
...
# --------------------------------------------------------------------------- #
def section6() -> None:
    late = build_dataset().model_copy(update={"fetched_at": datetime(2026, 9, 7, 22, 0, tzinfo=TZ)})
    try:
    ...
    cand_good = SpotCandidate(
        spot_id="g", name_ar="بقعة جيدة", location=good.location, spot=good.spot
    )
    cand_weak = SpotCandidate(
        spot_id="w", name_ar="بقعة عاصفة", location=weak.location, spot=weak.spot
    )
    cand_short = SpotCandidate(
        spot_id="s", name_ar="بقعة ناقصة", location=short.location, spot=short.spot
    )
    ...
    check(
        ...
        ranked.ranked[0].spot_id == "g"
        and ranked.ranked[0].decision == DecisionLevel.GO
...
def section7() -> None:
    ...
    cases = [
        (
            "steady wind 33 km/h (caution band 32-44)",
            {"wind_speed_kmh": 33.0, "wind_direction_deg": 90.0},
        ),
        ...
        (
            "steep chop 0.7 m / 2.5 s (steepness caution)",
            {"wave_height_m": 0.7, "wave_period_s": 2.5},
        ),
        (
        ...
        (
            "convective instability (cape 1500 + rain)",
            {"cape_jkg": 1500.0, "precipitation_mm": 1.0, "precipitation_probability_pct": 40.0},
        ),
