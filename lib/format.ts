...
const WIND_SECTORS = ["شمالية", "شمالية شرقية", "شرقية", "جنوبية شرقية", "جنوبية", "جنوبية غربية", "غربية", "شمالية غربية"];

/** الاسم الكامل لاتجاه قدوم الريح (8 قطاعات). */
export function windSectorName(value: number | null | undefined): string {
  if (value === null || value === undefined || Number.isNaN(value)) return "غير معروف";
  const sector = Math.round(((value % 360) + 360) % 360 / 45) % 8;
  return WIND_SECTORS[sector];
}
...
export function swellExposureLabel(value: string): string {
  return {
    direct: "مكشوف للتازيب (داخل مباشرة)",
    oblique: "مكشوف للتازيب (مائل)",
    alongshore: "تازيب موازٍ للشاطئ",
    inconsistent: "محمي من التازيب",
    unknown: "لا تازيب يُذكر",
  }[value] ?? value;
}
