...
import { buildPrimaryReason } from "@/lib/reason";
...
type ReportTab = "summary" | "windows" | "fish" | "feasibility" | "references" | "signals";
...
function activityWhyLine(species: SpeciesActivity): string {
  const scored = species.periods.filter((period) => period.score !== null);
  if (!scored.length) return "";
  const peak = scored.reduce((best, period) =>
    (period.score ?? 0) > (best.score ?? 0) ? period : best,
  );
  if (!peak.factors_ar.length) return "توازن بلا أفضلية قوية";
  return peak.factors_ar.join(" · ");
}
...
function buildFieldGuidance(
  ...
  const rigAdvice = buildRigAdvice(result);
  if (rigAdvice) {
    guidance.push(`نصيحة للمنتاج: ${rigAdvice}`);
  }
...
function buildRigAdvice(result: DecisionResponse): string | null {
  const field = result.field_feasibility;
  const tangling = [
    field.holding_difficulty,
    field.fouling_transport_potential,
    field.holding_breakdown?.longshore_current ?? "unknown",
    field.holding_breakdown?.orbital_motion ?? "unknown",
    field.holding_breakdown?.return_flow ?? "unknown",
    field.holding_breakdown?.tidal_current ?? "unknown",
  ];
  const tangled = tangling.some((value) => value === "moderate" || value === "high");
  if (!tangled) return null;
  return (
    "لتقليل التشابك (وليس للتثبيت): قصّر الإمبيلة + خفف وزن الرصاص + ارفع السنارة عن القاع قليلاً. "
    + "وإذا كان الهدف تثبيت الرصاص ضد جرّ الموج أو التيار، فزد الوزن ضمن قدرة القصبة."
  );
}

function windowSpeciesLine(
  result: DecisionResponse,
  ...
  const top = rows[0];
  return `الأنشط في هذه النافذة: ${top.label} (${top.average}/100 مؤشر نسبي غير معاير)`;
}
...
function buildReportText(
  ...
  const rigAdvice = buildRigAdvice(result);
  const primaryReason = buildPrimaryReason(result, cautions);
  const geometryConflict = hours.some((hour) => hour.derived.wave_incidence === "inconsistent");
  ...
  const lines = [
    ...
    `• السبب الرئيسي: ${primaryReason}`,
    `• قابلية التنفيذ المادي: ${result.field_feasibility.score}/100 (${fieldFeasibilityLabels[result.field_feasibility.status]})${geometryConflict ? " · قيد تحقق ميداني (اتجاه الشاطئ مشتبه به)" : ""} · ثقة الفحص ${result.field_feasibility.confidence}/100 — تقدير آلي وليس رصداً في البقعة.`,
    `• فحص التنفيذ: صعوبة تثبيت الخط ${potentialLabels[result.field_feasibility.holding_difficulty]} · احتمال الصوفة/الحطام ${potentialLabels[result.field_feasibility.fouling_transport_potential]} (${result.field_feasibility.fouling_evidence_count} قرائن) · احتمال العكارة ${potentialLabels[result.field_feasibility.turbidity_potential]} (${result.field_feasibility.turbidity_evidence_count} قرائن) · تيار ساحبي محتمل ${potentialLabels[result.field_feasibility.rip_current_potential]}`,
    "• مؤشرا الصوفة والعكارة تنبيه للمعاينة الميدانية فقط؛ لا يمنعان وحدهما نافذة اجتازت السلامة بلا رصد فعلي.",
    `• مؤشر الفرصة النسبي: ${result.opportunity_score}/100 — مؤشر ترتيب وليس نسبة نجاح أو ضمان مصيد.`,
    `• ثقة مدخلات الإرشاد: ${result.confidence.score}/100 (${confidenceLabel(result.confidence.score)}) — تقيس اكتمال البيانات واتساقها وأفقها، وليست دقة ميدانية متحققة.`,
    ...(result.confidence.breakdown_ar[0] ? [`• ${result.confidence.breakdown_ar[0]}`] : []),
    `• الهدف: ${targetLabels[request.angler.target_species]}`,
    "• الطعم والمونتاج: لا يُحسمان من التوقع؛ يلزم رصد محلي للفرائس والقاع ومدخلات العتاد.",
    ...
    ...(request.spot.orientation_evidence ? [
      `• دليل الاتجاه: مماس الساحل ${request.spot.orientation_evidence.coastline_tangent_deg.toFixed(1)}° · مسافة الساحل ${request.spot.orientation_evidence.coastline_distance_m.toFixed(1)} م · خادم ${request.spot.orientation_evidence.server} · نصف بحث ${request.spot.orientation_evidence.search_radius_m} م · ثقة ${orientationConfidenceLabels[request.spot.orientation_evidence.confidence]}.`,
    ] : []),
    "• تعريف الاتجاه: زاوية من نقطة الوقوف نحو البحر المفتوح؛ جهة الريح والموج المعروضة هي جهة القدوم، والمحرك يحسب الفرق تلقائياً.",
    ...
    ...(result.current_weather_observation ? [
      ...
      `• صلاحية الرصد الزمنية: METAR لقطة ${formatGeneratedAt(result.current_weather_observation.observed_at)} فقط${leadWindow ? `؛ لا يصف نافذة ${formatWindowRange(leadWindow.start, leadWindow.end)} ولا توجد مقارنة رصد/نموذج لساعاتها.` : "؛ لا يصف أي نافذة لاحقة."}`,
    ...
    ...result.tide_events.map((event) => `• ${event.kind === "high" ? "المد يعلى (قمة)" : "المد ينزل (قاع)"}: ${formatHour(event.time)} · ${event.level_msl_m.toFixed(2)} م`),
    "• حركة المياه هنا من نموذج مستوى البحر والتيار؛ لا نعرض عبوراً قمرياً أو فترات سولونار كأنها مدّ مؤكد.",
    ...(result.advanced_signals_ar && result.advanced_signals_ar.length > 0
      ? [
          "",
          "1أ. الإشارات المتقدمة (سياق إضافي لا يغيّر القرار ولا السلامة)",
          ...result.advanced_signals_ar.map((signal) => `• ${signal}`),
        ]
      : []),
    "",
    "2. نوافذ الحصة",
    ...(result.recommended_windows.length
      ? result.recommended_windows.flatMap((window, index) => {
          const speciesLine = windowSpeciesLine(result, window);
          const base = `• ${index + 1}. ${formatWindowRange(window.start, window.end)} · ${shortDecisionLabels[window.safety]} · تنفيذ ${window.field_score}/100 · فرصة نسبية ${window.opportunity_score}/100 · ثقة مدخلات ${window.confidence_score}/100 — ${window.headline_ar}`;
          return speciesLine ? [base, `  ${speciesLine}.`] : [base];
        })
    ...
    ...(result.recommended_windows.length && result.avoid_windows.length
      ? ["• وجود فترة رعد/خطر لا يلغي نافذة منفصلة اجتازت البوابات؛ لا تمدد النافذة إلى فترة التجنب."]
      : []),
    ...(result.species_activity && result.species_activity.length > 0
      ? [
          ...
          ...result.species_activity.map((species) => {
            ...
            const best = ranked[0];
            return `• ${species.label_ar}: أنشط فترة ${best.label_ar} (${best.range_ar}) بدرجة ${best.score}/100${best.peak_hour_ar ? ` · أفضل ساعة ${best.peak_hour_ar}` : ""}${best.top_factor_ar ? ` · العامل الأبرز: ${best.top_factor_ar}` : ""}`;
          }),
          "• هذا مؤشر ترتيب نسبي غير معاير، وليس احتمالاً إحصائياً لوجود السمك.",
        ]
      : []),
    "",
    "3. قابلية التنفيذ المادي وثقة الفحص",
    `• الحالة: ${fieldFeasibilityLabels[result.field_feasibility.status]} · ${result.field_feasibility.score}/100${geometryConflict ? " · قيد تحقق ميداني" : ""} · ثقة الفحص ${result.field_feasibility.confidence}/100 — الرقم للترتيب بين النوافذ من أدلة منخفضة الثقة، وليس دقة ميدانية.`,
    `• صعوبة تثبيت الخط: ${potentialLabels[result.field_feasibility.holding_difficulty]}`,
    ...(result.field_feasibility.holding_breakdown
      ? [
          `• أسباب صعوبة التثبيت: جر جانبي ${potentialLabels[result.field_feasibility.holding_breakdown.longshore_current]} · حركة القاع المدارية ${potentialLabels[result.field_feasibility.holding_breakdown.orbital_motion]} (${result.field_feasibility.holding_breakdown.orbital_velocity_band_ms ?? "غير متوفرة"} م/ث، عمق مفترض ${result.field_feasibility.holding_breakdown.depth_band_m} م) · رجوع للبحر ${potentialLabels[result.field_feasibility.holding_breakdown.return_flow]} · تيار المد ${potentialLabels[result.field_feasibility.holding_breakdown.tidal_current]} · الأهم: ${holdingMechanismLabels[result.field_feasibility.holding_breakdown.dominant]}.`,
        ]
      : []),
    ...(rigAdvice ? [`• نصيحة للمنتاج: ${rigAdvice}`] : []),
    ...
    `• احتمال العكارة: ${potentialLabels[result.field_feasibility.turbidity_potential]} من ${result.field_feasibility.turbidity_evidence_count} قرائن تشغيلية؛ لا توجد قراءة NTU ولا يمنع وحدها.`,
    ...
    ...(fieldContext ? [
      `• سياق آخر 48 ساعة عند ${formatHour(fieldContext.time)}: مطر ${metricValue(fieldContext.derived.rain_48h_mm, 1, " مم")} (تغطية ${fieldContext.derived.rain_data_hours_48h}/48) · ريح بحرية ${fieldContext.derived.onshore_wind_fraction_48h === null ? "غير متوفر" : `${Math.round(fieldContext.derived.onshore_wind_fraction_48h * 100)}%`} ودفع ${metricValue(fieldContext.derived.onshore_wind_impulse_48h_kmh_h, 1, " كم/س·س")} (تغطية ${fieldContext.derived.wind_data_hours_48h}/48) · طاقة موج تراكمية ${metricValue(fieldContext.derived.wave_energy_integral_48h, 1, " Hs²T·h")} وساعات قوية ${fieldContext.derived.strong_wave_hours_48h} (تغطية ${fieldContext.derived.wave_energy_data_hours_48h}/48) · تاريخ ${fieldContext.derived.history_hours_72h}/72 ساعة.`,
    ] : []),
    ...unique(result.field_feasibility.reasons_ar).map((reason) => `• تفسير: ${reason}`),
    ...unique(result.field_feasibility.limitations_ar).map((limitation) => `• حد: ${limitation}`),
    "",
    "4. التفكيك الزمني",
    ...
    ] : [`• ${period.label} (${period.range}): انقضت هذه الفترة اليوم قبل وقت إنشاء التقرير؛ الأداة تقيّم الساعات المقبلة فقط.`]),
    "",
    "5. ميزان العوامل",
    "• ملاحظة: العوامل أدناه مجمعة من اليوم كله؛ ظهور الرعد هنا لا يعني كل الساعات، وفترات التجنب في القسم السابق هي المرجع.",
    ...
    ...(hours.length ? [
      `• محور الموجب على طول الساحل: bearing ${(hours[0].derived.alongshore_positive_bearing_deg).toFixed(0)}° = (اتجاه البحر + 90) مع باقي القسمة على 360. الحقل الموقّع يبيّن الجهة.`,
      ...
      `• النطاقات: دفع موازٍ موقّع ${signedMetricRange(numericValues(hours, (hour) => hour.derived.wind_alongshore_signed_component_kmh), 2, " كم/س")} · إجهاد الريح ${metricRange(numericValues(hours, (hour) => hour.derived.wind_stress_pa), 4, " Pa")} · دفع موج موقّع ${signedMetricRange(numericValues(hours, (hour) => hour.derived.alongshore_wave_signed_proxy), 2, " Hs²T")} · زاوية التقاطع ${metricRange(numericValues(hours, (hour) => hour.derived.wave_component_angle_deg), 0, "°")}.`,
      ...(geometryConflict ? ["• دفع الموج الموازي معطَّل لأن جهة الموج غير بحرية (تعارض هندسي): لا يُفكَّك الموج على محور شاطئ مشتبه به."] : []),
      "• معنى الرموز: Δ = الفرق بين جهة القدوم واتجاه الشاطئ · V⊥ = الدفع نحو الشاطئ · V∥ = الدفع الموازي للساحل · τ = إجهاد الريح على سطح البحر · L₀ = طول الموجة العميقة · Hs/L₀ = انحدار الموج · H²T = طاقة الموج · R = تماسك اتجاه الريح. لا عتبة قرار لهذه التشخيصات (قياس فني فقط).",
      "• الصيغ: V⊥=VcosΔ (دفع نحو الشاطئ) · V∥=−VsinΔ (دفع موازٍ) · L₀=gT²/(2π) (طول الموجة العميقة، تقريب مياه عميقة) · Hs/L₀ الانحدار · τ⃗=ρair Cd |U10|U⃗10 · R=|ΣUi eᶦᶿⁱ|/ΣUi.",
      ...hours.map((hour) =>
        `• ${formatHour(hour.time)} | الريح: فرق الاتجاه ${diagnosticValue(hour.derived.wind_angle_deg, 1, "°")} · دفع نحو الشاطئ ${signedDiagnosticValue(hour.derived.wind_shoreward_component_kmh, 2)} · دفع موازٍ ${signedDiagnosticValue(hour.derived.wind_alongshore_signed_component_kmh, 2)} كم/س · إجهاد ${diagnosticValue(hour.derived.wind_stress_pa, 4)} (نحو الشاطئ ${signedDiagnosticValue(hour.derived.wind_stress_shoreward_pa, 4)} · موازٍ ${signedDiagnosticValue(hour.derived.wind_stress_alongshore_pa, 4)}) Pa | الموج: فرق الاتجاه ${diagnosticValue(hour.derived.wave_angle_deg, 1, "°")} · طول الموجة العميقة ${diagnosticValue(hour.derived.deep_water_wavelength_m, 1, " م")} · الانحدار ${diagnosticValue(hour.derived.wave_steepness, 4)} · الطاقة ${diagnosticValue(hour.derived.wave_energy_proxy, 2)} · دفع موازٍ ${signedDiagnosticValue(hour.derived.alongshore_wave_signed_proxy, 2)} | موج ريح/سويل: زاوية ${diagnosticValue(hour.derived.wave_component_angle_deg, 0, "°")} · حصة الأضعف ${hour.derived.wave_component_secondary_energy_share === null ? "Unknown" : `${(hour.derived.wave_component_secondary_energy_share * 100).toFixed(0)}%`} | التيار: نحو الشاطئ ${signedDiagnosticValue(hour.derived.current_cross_shore_kmh, 2)} · موازٍ ${signedDiagnosticValue(hour.derived.current_alongshore_kmh, 2)} كم/س | تماسك اتجاه الريح ${diagnosticValue(hour.derived.wind_direction_coherence_6h, 2)} (${hour.derived.wind_direction_data_hours_6h}/6 ساعات).`,
      ),
    ] : []),
    "",
    ...
    `• قرار آلي ${result.factor_coverage.automated_decision} · سياق آلي ${result.factor_coverage.automated_context} · مؤشر يحتاج تحققاً ميدانياً ${result.factor_coverage.proxy_requires_field_check} · ميداني/خارجي ${result.factor_coverage.field_or_external_required} · مستبعد ${result.factor_coverage.excluded_unsupported}.`,
    ...
    ...unique(result.confidence.breakdown_ar).map((item) => `• تفكيك الثقة: ${item}`),
    ...unique(result.confidence.reasons_ar).map((reason) => `• سبب ثقة: ${reason}`),
    ...unique(result.limitations_ar).map((limitation) => `• حد: ${limitation}`),
...
export function SpotReport({ result, request, stale = false, onClose }: SpotReportProps) {
  ...
  const cautions = useMemo(
    () => factors
      .filter((factor) => factor.severity === "caution")
  ...
  const reportText = useMemo(
    () => buildReportText(result, request, periods, obstacles, positives, cautions, guidance, stale),
    [cautions, guidance, obstacles, periods, positives, request, result, stale],
  );
  ...
  const swellExposure = dominant(result.hourly.map((hour) => hour.derived.swell_incidence).filter((value) => value !== "unknown"));
  const swellMax = maximum(numericValues(result.hourly, (hour) => hour.forecast.swell_height_m));
  ...
  const handleWhatsAppShare = () => {
    ...
    const shareText = reportText.length > maxCharacters
      ? `${reportText.slice(0, maxCharacters)}\n\n… التقرير مختصر بسبب حد المشاركة؛ نزّل TXT من Peche TN للتقرير الكامل.`
      : reportText;
    ...
    <section className="spot-report" id="spot-report" aria-labelledby="spot-report-title" aria-live="off">
      ...
      <section className="report-section" aria-labelledby="report-summary-title" hidden={reportTab !== "summary"}>
        ...
        <div className="report-summary-grid">
          ...
          <article className="report-field-summary"><small>قابلية التنفيذ المادي</small><strong><bdi>{result.field_feasibility.score}/100</bdi></strong><p>{fieldFeasibilityLabels[result.field_feasibility.status]} · ثقة الفحص {result.field_feasibility.confidence}/100.</p></article>
      ...
      <section className="report-section" aria-labelledby="report-water-title" hidden={reportTab !== "windows"}>
        ...
        {swellExposure && swellExposure !== "unknown" && (
          <p className="report-method-note"><Info size={15} /><span><strong>انكشاف التازيب:</strong> {swellExposureLabel(swellExposure)}{swellMax === null ? "" : ` · أقصى ارتفاع سويل ${swellMax.toFixed(2)} م`}. هندسة زوايا من نموذج موج، لا قياس كسرة ولا ادعاء مصيد؛ تبديل شاطئ محمي خيار تشغيلي لا قرار آلي.</span></p>
        )}
      </section>
              ...
              <div className="report-species-activity">
                ...
                {result.species_activity.map((species) => (
                  <div className="species-activity-hourly" key={`hourly-${species.species}`}>
                    ...
                    <p className="species-activity-why"><small>لماذا هذا الرقم</small><span>{activityWhyLine(species)}</span></p>
                  </div>
                ))}
      ...
      <section className="report-section report-field-feasibility" aria-labelledby="report-feasibility-title" hidden={reportTab !== "feasibility"}>
        ...
        {result.gear_recommendation && <GearPanel gear={result.gear_recommendation} />}
      ...
      <section className="report-section report-reference-section" aria-labelledby="report-reference-title" hidden={reportTab !== "references"}>
        ...
        {result.coastal_water_context && <div className={`report-water-context water-${result.coastal_water_context.availability}`} data-testid="report-water-context">
          ...
          <p><Info size={14} /> أخذت العينة عند نقطة ثابتة على بعد 1 كم باتجاه البحر، بلا بحث جانبي عن بكسل ملائم. TUR بوحدة FNU وليست NTU، والمنتج لا يرصد الصوفة أو التيار الساحبي أو ساعة الرحلة المستقبلية، وCHL لا تشخّص ازدهاراً ضاراً أو نشاط السمك. قراءة TUR الساتلية قناة منفصلة عن سلم القرائن التشغيلية الخمس ولا تُحسب ضمنه كعائلة قرائن للعكارة.</p>
        </div>}
      </section>
      ...
      <section className="report-section report-signals" aria-labelledby="report-signals-title" hidden={reportTab !== "signals"}>
        <div className="report-section-title"><span>✦</span><div><h3 id="report-signals-title">إشارات متقدمة</h3><p>مؤشرات محسوبة من داتا حقيقية — لا تغيّر القرار ولا السلامة.</p></div></div>
        {result.advanced_signals_ar && result.advanced_signals_ar.length > 0 ? (
          <div className="report-signal-list">
            {result.advanced_signals_ar.map((signal) => (
              <article key={signal} className="report-signal-item">
                <span>{signal}</span>
              </article>
            ))}
          </div>
        ) : (
          <p className="report-empty">لا توجد إشارات متقدمة في هذه الاستجابة.</p>
        )}
        <div className="report-method-note"><Info size={15} /><span>الإشارات أعلاه سياق إضافي للصياد: فلك حقيقي، سلاسل النموذج، وموسمية منشورة. أي قرينة شعبية أو موسمية معلنة صراحة كقرينة، ولا تدخل القرار الثنائي أو أي بوابة سلامة.</span></div>
      </section>
