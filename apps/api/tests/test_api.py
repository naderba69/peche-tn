...
def test_health_and_methodology() -> None:
    with TestClient(app) as client:
        ...
        assert health.json()["version"] == "1.15.0"
        assert health.json()["engine_version"] == "1.7.0"
        assert health.json()["schema_version"] == "3.13"
...
def test_offline_evaluate_endpoint(
    ...
    assert body["moonset"] is None
    assert body["recommended_windows"]
    assert isinstance(body["advanced_signals_ar"], list)
    assert body["advanced_signals_ar"]
    assert body["advanced_signals_ar"][0].startswith("كل ما يلي")
