...
// يطابق RankSpotsRequest.spots (max_length=24) في المحرك.
const MAX_SPOTS_PER_SWEEP = 24;
...
export default function WilayasPage() {
  ...
  const tooManySpots = spots.length > MAX_SPOTS_PER_SWEEP;
  ...
  const run = async () => {
    if (!spots.length || tooManySpots) return;
    setLoading(true);
    setError(null);
    setResult(null);
      ...
      <section className="wilaya-tools" id="wilaya-tools" aria-label="أدوات الترتيب">
        ...
        <div className="wilaya-controls">
          ...
          <button
            ...
            type="button"
            disabled={loading || !spots.length || tooManySpots}
            onClick={run}
          >
        ...
        {tooManySpots && (
          <p className="wilaya-empty" role="status">
            اخترت {spots.length} بقعة والحد الأقصى {MAX_SPOTS_PER_SWEEP} في المسح الواحد. أزل ولاية أو أكثر.
          </p>
        )}
        {loading && spots.length > MAX_SPOTS_PER_SWEEP / 2 && (
          <p className="wilaya-empty" role="status">
            مسح {spots.length} بقعة يأخذ وقتاً أطول — البيانات تُجلب على دفعات محدودة لتفادي خنق المزوّد.
          </p>
        )}
      </section>
    ...
    <li className={`wilaya-card ${isGo ? "card-go" : "card-nogo"}`}>
      ...
      <div className="wilaya-card-main">
        ...
        <div className="wilaya-metrics">
          ...
          {spot.swell_exposure ? (
            <span><small>انكشاف التازيب</small><strong>{swellExposureLabel(spot.swell_exposure)}</strong></span>
          ) : null}
        </div>
