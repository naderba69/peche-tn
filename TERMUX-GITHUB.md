...
> الإصدار الحالي: **1.15.0** (المحرك `1.7.0`، schema `3.13`). الملف المطلوب: `peche-tn-v1.15.0.zip`.

تحتاج ملفاً واحداً فقط:

- `peche-tn-v1.15.0.zip`
...
A="$HOME/storage/downloads/peche-tn-v1.15.0.zip"
...
rm -rf "$HOME/peche-tn"
rm -f "$HOME/storage/downloads/peche-tn-v1.14.2.zip"
unzip -q "$A" -d "$HOME/peche-tn"
cd "$HOME/peche-tn"
