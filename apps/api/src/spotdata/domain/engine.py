...
from .report import build_fisherman_report
from .species import (
    MONTHS_AR,
    PRESENCE_LABELS_AR,
    SEA_STATE_LABELS_AR,
    SHORE_MATCH_DELTA,
    SHORE_MISMATCH_DELTA,
    SOURCE_REGISTRY,
    SPECIES_PROFILES,
    SST_OUTSIDE_TOLERANCE_DELTA,
    SST_PREFERRED_DELTA,
    ZONE_LABELS_AR,
    SpeciesProfile,
    classify_sea_state,
...
_ORIENTATION_QUALITY = {
    # Ranked by how closely the value tracks the true shore normal at the stand:
    # field-measured > user-typed > user-placed on map > automated OSM (Overpass)
    # > default estimate. Overpass sits below "map" because it is a 3 km-radius
    # automated coastline normal, while a user placing the arrow on the map is
    # usually standing at the spot (audit 2026-09-14, item A1).
    "surveyed": 0.95,
    "manual": 0.85,
    "map": 0.78,
    "overpass": 0.70,
...
def _dominant_swell_incidence(hourly: list[HourDecision]) -> WaveIncidence | None:
    """Majority swell-shore incidence across hours with meaningful swell.

    Only hours whose swell height reaches the same 0.5 m floor used for the
    exposure factor are counted, so a negligible swell never labels a spot as
    exposed or sheltered. Returns None when no hour qualifies.
    """
    counts: dict[WaveIncidence, int] = {}
    for hour in hourly:
        swell = hour.forecast.swell_height_m
        incidence = hour.derived.swell_incidence
        if swell is not None and swell >= 0.5 and incidence is not WaveIncidence.UNKNOWN:
            counts[incidence] = counts.get(incidence, 0) + 1
    if not counts:
        return None
    return max(counts, key=lambda incidence: counts[incidence])
...
def _species_hour_activity(
    ...
    *,
    ...
    tide_delta: int = 0,
    lunar_day_delta: int = 0,
) -> tuple[int, list[tuple[str, int]]]:
    """Relative activity score (0-100) for one species at one hour.

    Base 50 plus documented, low-weight priors. Factors whose *values* are
    computed from real data (feeding window from modelled sunrise/sunset, water
    temperature, seasonal availability from published studies (dominant), shore
    match, sea-state fit, the 3-hour pressure trend from the hPa series, the
    day's spring/neap from the sea-level series, and astronomically-computed
    moonlight at night plus the traditional lunar-day prior). Never a catch
    ...
    if profile.preferred_sst_c is not None and sst is not None:
        ...
        if pref_low <= sst <= pref_high:
            score += SST_PREFERRED_DELTA
            contributions.append(("حرارة الماء مفضلة", SST_PREFERRED_DELTA))
        elif not tol_low <= sst <= tol_high:
            score += SST_OUTSIDE_TOLERANCE_DELTA
            contributions.append(("حرارة الماء بعيدة", SST_OUTSIDE_TOLERANCE_DELTA))
    if profile.preferred_shores:
        if shore_type in profile.preferred_shores:
            score += SHORE_MATCH_DELTA
            contributions.append(("القاع ملائم", SHORE_MATCH_DELTA))
        else:
            score += SHORE_MISMATCH_DELTA
            contributions.append(("القاع خارج التفضيل", SHORE_MISMATCH_DELTA))
    ...
    # Day-level shore-proximity from the modelled spring/neap (sea-level series,
    # D9): spring (حيّة) means stronger water movement pulling fish shoreward.
    if tide_delta:
        score += tide_delta
        contributions.append(
            (
                "مد قوي: سمك أقرب للشاطئ" if tide_delta > 0 else "مد ضعيف: سمك أبعد عن الشاطئ",
                tide_delta,
            )
        )
    # Traditional lunar-day prior (أيام حياة/موت): the folk calendar keys on the
    # days near the new/full moon. Disclosed folklore, kept small (+2), never the
    # D9 spring/neap derivation and never a decision input.
    if lunar_day_delta:
        score += lunar_day_delta
        contributions.append(("يوم قمري قرب البدر/المحاق (قرينة شعبية)", lunar_day_delta))
    # 3-hour pressure trend, computed from the modelled hPa series: a gently
    # falling barometer precedes frontal activity; a sharp rise follows it.
    # Disclosed as an uncalibrated prior — the *value* is data, the weight is not.
    pressure_change_3h = hour.derived.pressure_change_3h_hpa
    if pressure_change_3h is not None:
        if pressure_change_3h <= -1.0:
            score += 3
            contributions.append(("هبوط ضغط 3س", 3))
        elif pressure_change_3h >= 1.5:
            score -= 3
            contributions.append(("صعود ضغط 3س", -3))
    # Moonlit night: the light channel (moonlight aids nocturnal feeding sight).
    # Astronomically computed; night-only, so daytime is unaffected. Note: within
    # the ±90 min twilight margin after sunset, is_night is still True, so a
    # bright moon can co-occur with the twilight bonus above — a documented small
    # overlap, not an error (audit 2026-09-14, item C6).
    if hour.derived.is_night and moon_illumination is not None and moon_illumination >= 0.7:
        score += 3
        contributions.append(("ليلة مقمرة", 3))
    return max(0, min(100, score)), contributions


def _tide_channel_delta(classification: str) -> int:
    """Day-level shore-proximity delta from the modelled spring/neap (D9/D11).

    The classification is computed from the sea-level series only (never the
    moon phase); spring = stronger movement = fish pushed shoreward (+3), neap =
    calmer sea (-3). The *value* is data, the weight is a disclosed prior.
    """
    if classification == "spring":
        return 3
    if classification == "neap":
        return -3
    return 0


def _lunar_day_delta(illumination: float) -> int:
    """Traditional 'أيام حياة/موت' lunar-day prior, disclosed and uncalibrated.

    The folk calendar treats the days around the new and full moon as living
    days. Kept as a small folklore prior (+2) explicitly separate from the D9
    spring/neap derivation; it never feeds the binary decision or any safety
    gate. Thresholds ±0.12 illumination ≈ ~2 days around each syzygy.
    """
    if illumination >= 0.88 or illumination <= 0.12:
        return 2
    return 0
...
def _activity_factors_ar(contributions: list[tuple[str, int]]) -> list[str]:
    """Signed, strongest-first list of a peak hour's score contributions."""
    ordered = sorted(contributions, key=lambda item: abs(item[1]), reverse=True)
    return [f"{label} {delta:+d}" for label, delta in ordered]


def _activity_periods(
    entries: list[tuple[HourActivity, list[tuple[str, int]]]],
) -> list[PeriodActivity]:
    periods: list[PeriodActivity] = []
    for key, label, range_ar, from_hour, to_hour in _ACTIVITY_PERIODS:
        members = [
            (item, contributions)
            for item, contributions in entries
            if _in_activity_period(item.time.hour, from_hour, to_hour)
        ]
        if not members:
            periods.append(PeriodActivity(key=key, label_ar=label, range_ar=range_ar, score=None))
            continue
        average_score = round(sum(item.score for item, _contrib in members) / len(members))
        peak_item, peak_contributions = max(members, key=lambda pair: pair[0].score)
        periods.append(
            PeriodActivity(
                key=key,
                label_ar=label,
                ...
                top_factor_ar=_activity_top_factor(peak_contributions),
                factors_ar=_activity_factors_ar(peak_contributions),
            )
        )
    return periods
...
def _activity_basis_ar(profile: SpeciesProfile) -> list[str]:
    lines = [
        "مؤشر نشاط نسبي (0-100) يبدأ من 50 ويجمع أوزاناً أولية موثقة: نافذة التغذية (غسق/ليل)، حرارة الماء، التوفر الموسمي من دراسات منشورة (الأثقل)، توافق القاع، حالة البحر، المد اليومي (حيّة/مات) من سلسلة مستوى البحر، اليوم القمري قرب البدر/المحاق (قرينة شعبية)، اتجاه الضغط خلال 3 ساعات من سلسلة hPa، وإضاءة القمر ليلاً (حساب فلكي).",
    ]
    if profile.label_ar == "صيد عام":
        lines.append(
            "صيد عام بلا ملف موسمي أو حراري أو قاع: يدخل في مؤشره بونص الغسق والليل والمد واليوم القمري والضغط وإضاءة القمر."
        )
    lines.append(
        "هذا مؤشر ترتيب نسبي وليس احتمالاً إحصائياً لوجود السمك أو وعداً بالمصيد؛ أوزان المد واليوم القمري والضغط وإضاءة القمر قرائن غير معايرة بقيم محسوبة من الداتا، وتحويل المؤشر كله إلى نسبة يتطلب سجل مصيد تونسي معايراً يشمل الحصص الصفرية."
    )
    return lines
...
class DecisionEngine:
    ...
    def evaluate(self, dataset: ForecastDataset) -> DecisionResponse:
        ...
        species_activity = self._species_activity(dataset, hourly, spring_neap)
        advanced_signals = self._advanced_signals(dataset, hourly, recommended)
        ...
        response = DecisionResponse(
            ...
            moonrise=dataset.moonrise,
            moonset=dataset.moonset,
            advanced_signals_ar=advanced_signals,
        ...
        response.fisherman_report = build_fisherman_report(dataset, response)
        return response
    ...
    def rank_spots(
        ...
        for candidate, dataset in datasets:
            if dataset is None:
                ranked.append(error_entry(candidate, "تعذر جلب بيانات هذه البقعة من المزوّد."))
                continue
            try:
                result = self.evaluate(dataset)
            except InsufficientDataError:
                ranked.append(error_entry(candidate, "لا توجد ساعات كافية في التاريخ المطلوب."))
                continue
            ...
            swell_exposure = _dominant_swell_incidence(result.hourly)
            species_summary = (
                [
    ...
    def _evaluate_hour(
        ...
        swell_incidence, swell_angle = classify_wave(
            hour.swell_direction_deg, dataset.spot.seaward_orientation_deg
        )
        ...
        previous_pressure_6h = pressure_by_time.get(hour.time - timedelta(hours=6))
        previous_pressure_12h = pressure_by_time.get(hour.time - timedelta(hours=12))
        ...
        pressure_change_6h = (
            hour.pressure_msl_hpa - previous_pressure_6h
            if hour.pressure_msl_hpa is not None and previous_pressure_6h is not None
            else None
        )
        pressure_change_12h = (
            hour.pressure_msl_hpa - previous_pressure_12h
            if hour.pressure_msl_hpa is not None and previous_pressure_12h is not None
            else None
        )
        pressure_change_24h = (
        ...
        solar_elevation = solar_elevation_deg(
            hour.time, dataset.location.latitude, dataset.location.longitude
        )
        ...
        derived = DerivedConditions(
            ...
            swell_incidence=swell_incidence,
            swell_angle_deg=_rounded(swell_angle, 1),
            ...
            pressure_change_6h_hpa=_rounded(pressure_change_6h, 1),
            pressure_change_12h_hpa=_rounded(pressure_change_12h, 1),
            pressure_change_24h_hpa=_rounded(pressure_change_24h, 1),
            solar_elevation_deg=_rounded(solar_elevation, 1),
    ...
    def _field_feasibility(
        ...
        # The gate keeps its calibrated sum-and-threshold combine (so the binary
        # decision is unchanged), but its alongshore channels now use the exact
        # same weights as the displayed four-mechanism breakdown, and the wind
        # channel follows the alongshore component (>=15 km/h) instead of a
        # direction-blind magnitude (>=25 km/h). This removes the old
        # "100/100 next to a متوسط sub-indicator" contradiction (audit
        # 2026-09-14) without touching safety thresholds or the no-go combine.
        holding_points = 0
        ...
        # A summary that sits below its own most-severe displayed cause is the
        # "منخفض/100 بجوار متوسط" contradiction (audit 2026-09-14, round 3).
        # Floor the gate level to the breakdown's dominant mechanism so the
        # number can never contradict its own explanation.
        breakdown = self._holding_breakdown(dataset, hour, derived)
        if breakdown.dominant != "none":
            ...
            if _POTENTIAL_SEVERITY[dominant_level] > _POTENTIAL_SEVERITY[holding]:
                holding = dominant_level
        ...
        # A MODERATE/HIGH holding mechanism caps the verbal status so the label
        # never contradicts its own sub-indicators ("قابلة مع ضبط العتاد" instead
        # of "مريحة ميدانياً" when the breakdown says متوسط).
        status = (
            FieldFeasibilityLevel.DIFFICULT
            if holding == PotentialLevel.HIGH
            else FieldFeasibilityLevel.WORKABLE
            if holding == PotentialLevel.MODERATE
            else FieldFeasibilityLevel.FAVORABLE
            if score >= 75
            else FieldFeasibilityLevel.WORKABLE
            if score >= 50
        ...
        if derived.wave_incidence == WaveIncidence.INCONSISTENT:
            reasons.append(
                "اتجاه الشاطئ مشتبه به (الموج قادم من جهة غير بحرية)؛ تقدير الجرّ الجانبي ودفع الريح الموازي غير موثوق حتى يتحقق المستخدم من جهة البحر ميدانياً."
            )
        return FieldFeasibilityResult(
            status=status,
            score=score,
    ...
    def _holding_breakdown(
        ...
        The alongshore channel uses the same weights as the `holding` gate, and
        the orbital / return / tidal channels are banded to the same wave-height
        and sea-level references as the gate, so no sub-indicator can sit above
        the summary it explains (audit 2026-09-14, round 3). The gate floors its
        own level to the dominant mechanism, closing the "منخفض/100 بجوار متوسط"
        ...
        if orbital_shallow is None or orbital_deep is None:
            ...
            orbital_band = f"{min(orbital_shallow, orbital_deep):.2f} إلى {max(orbital_shallow, orbital_deep):.2f}"
            # Bands aligned to the gate's wave-height channel (audit 2026-09-14,
            # round 3): at the 1.5 m shallow reference, a ~1.2 m wave (the gate's
            # MODERATE step) drives ≈0.9 m/s near the bed and a ~1.8 m wave (the
            # safety no-go step) ≈1.4 m/s. The old bands (0.25/0.50) labelled an
            # ordinary 0.5-0.6 m wave "مرتفع" while the gate said "منخفض".
            if orbital_shallow >= 1.40:
                orbital = PotentialLevel.HIGH
            elif orbital_shallow >= 0.90:
                orbital = PotentialLevel.MODERATE
            else:
                orbital = PotentialLevel.LOW

        # 3. Return flow under the breaker: mirrors the gate's wave-height channel
        # (0.5/0.8/1.2 m → +1/+2/+3) so this cause can no longer show
        # "متوسط/مرتفع" while the gate says "منخفض". Directional drag is already
        # covered by the longshore mechanism; this channel isolates the
        # cross-shore push magnitude.
        return_points = 0
        ...
        # 4. Tidal current: recalibrated so a typical Mediterranean day
        # (rate ≈0.18 m/h, range ≈0.35 m) is LOW while a Gabès spring tide
        # (range ≥1.5 m, rate ≥0.3 m/h) is MODERATE. The old bands labelled the
        # ordinary Mediterranean signal "متوسط" while the gate said "منخفض".
        tidal_points = 0
        if derived.tide_state in {TideState.RISING, TideState.FALLING}:
            tidal_points += 1
        if derived.sea_level_rate_m_per_h is not None:
            if abs(derived.sea_level_rate_m_per_h) >= 0.30:
                tidal_points += 2
            elif abs(derived.sea_level_rate_m_per_h) >= 0.15:
                tidal_points += 1
    ...
    def _safety(
        ...
        if (
            ...
            and thresholds.gust_no_go_kmh * 0.90 <= hour.wind_gust_kmh < thresholds.gust_no_go_kmh
        ):
            near_limit_codes.append("gust")
        if (
            hour.wave_height_m is not None
            and thresholds.wave_no_go_m * 0.90 <= hour.wave_height_m < thresholds.wave_no_go_m
        ):
    ...
    def _opportunity(
        ...
        if (
            hour.swell_height_m is not None
            and hour.swell_height_m >= 0.5
            and derived.swell_incidence in {WaveIncidence.DIRECT, WaveIncidence.OBLIQUE}
        ):
            context(
                "direct_swell_exposure",
                "تازيب داخل على البقعة",
                f"{WAVE_INCIDENCE_AR[derived.swell_incidence]} · ارتفاع {hour.swell_height_m:.2f} م",
                "اتجاه السويل (التازيب) بالنسبة لواجهة الشاطئ: بقعة مكشوفة تستقبل الموج الطويل مباشرة فيكون الرمي وقراءة الضربة أصعب. هندسة زوايا من نموذج، لا قياس كسرة ولا ادعاء مصيد، وبوابة ارتفاع الموج الكلي تبقى مرجع السلامة.",
                rule_nature=RuleNature.OPERATIONAL_PROXY,
            )
        elif (
            hour.swell_height_m is not None
            and hour.swell_height_m >= 0.5
            and derived.swell_incidence == WaveIncidence.INCONSISTENT
        ):
            context(
                "sheltered_from_swell",
                "البقعة محمية من التازيب",
                f"{WAVE_INCIDENCE_AR[derived.swell_incidence]} · ارتفاع {hour.swell_height_m:.2f} م",
                "اتجاه السويل قادم من خلف الواجهة البحرية للبقعة، أي أن هذا الشاطئ في وضع احتماء نسبي؛ راحة رمي أعلى، والحكم النهائي للمعاينة الميدانية.",
                rule_nature=RuleNature.OPERATIONAL_PROXY,
            )
        ...
        if field_feasibility.fouling_transport_potential == PotentialLevel.HIGH:
            ...
            onshore_fraction_text = (
                "غير متوفرة"
                if derived.onshore_wind_fraction_12h is None
                else f"استمرار ريح بحرية {derived.onshore_wind_fraction_12h * 100:.0f}%"
            )
            add(
        ...
        if species.preferred_shores:
            if dataset.spot.shore_type in species.preferred_shores:
                add(
                    "species_shore_match",
                ...
                add(
                    "species_shore_mismatch",
                    "نوع القاع خارج تفضيل ملف السمكة",
                    SHORE_MISMATCH_DELTA,
                    species.label_ar,
                    "prior سلبي صغير غير معاير محلياً؛ لا يلغي نافذة اجتازت السلامة.",
                    basis=FactorBasis.SPOT_PROFILE,
                    rule_nature=RuleNature.EXPERT_PRIOR,
                )
    ...
    def _confidence(
        ...
        if derived.wave_incidence == WaveIncidence.INCONSISTENT:
            score = min(score, 55)
        # A station/model divergence is a snapshot near the fetch time, so it only
        # caps the confidence of hours close to the observation — the same ±2 h
        # window used by _apply_observation_context (audit 2026-09-14, item A2).
        observation = dataset.current_weather_observation
        station_divergence_applies = (
            observation_comparison.affects_confidence
            and observation is not None
            and abs((hour.time - observation.observed_at).total_seconds()) <= 2 * 3600
        )
        if station_divergence_applies:
            score = min(score, 55)
        ...
        if station_divergence_applies:
            reasons.append(
                "يوجد اختلاف مهم بين رصد محطة الطقس القريبة والنموذج عند وقت الجلب؛ خُفضت ثقة هذه الساعة القريبة من زمن الرصد ولم تُنقل قراءة المطار إلى الشاطئ."
            )
        ...
        return ConfidenceResult(
            ...
            reasons_ar=reasons,
            breakdown_ar=_confidence_breakdown(
                completeness=completeness,
                horizon_quality=horizon_quality,
                source_quality=source_quality,
                orientation_quality=orientation_quality,
                critical_missing=[_FIELD_LABELS_AR[name] for name in critical_missing],
                wave_inconsistent=derived.wave_incidence == WaveIncidence.INCONSISTENT,
                station_divergence=station_divergence_applies,
            ),
        )
    ...
    def _overall_field_feasibility(
        ...
        if any(item.derived.wave_incidence == WaveIncidence.INCONSISTENT for item in covered):
            reasons.append(
                "اتجاه الشاطئ مشتبه به (الموج قادم من جهة غير بحرية)؛ تقدير الجرّ الجانبي ودفع الريح الموازي غير موثوق حتى يتحقق المستخدم من جهة البحر ميدانياً."
            )
        return FieldFeasibilityResult(
    ...
    def _fouling_evidence(
        ...
        official_fouling = [
            report for report in valid if report.kind == "fouling" and report.source == "official"
        ]
        user_fouling = [
            report
            for report in valid
        ...
        if official_fouling:
            ...
            level = max(level, 4)
            basis.append(
                f"{len(user_fouling)} بلاغات حديثة متسقة من نفس البقعة؛ الصوفة مرشحة للقاهر."
            )
        elif user_fouling:
            level = max(level, 3)
    ...
    def _trip_ruin_factors(
        ...
        if water is not None and water.availability == "available" and water.valid_time is not None:
            age = water.age_hours
            water_age_ar = (
        ...
        tur = (
            water.turbidity_fnu if water is not None and water.availability == "available" else None
        )
        spm = (
        ...
        fresh_turbidity = [
            ...
            if r.kind == "turbidity" and _report_freshness(r.reported_at, dataset.fetched_at)[0]
        ]
        if fresh_turbidity and any(r.severity == "dense" for r in fresh_turbidity):
            ...
            if tur >= _TUR_HIGH_FNU:
                tur_level = "high"
                turbidity_evidence.append(
                    f"إشارة ساتلية لتعكر مرتفع (TUR {tur:.1f} FNU) — تنبيه لا منع آلي."
                )
            ...
            turbidity_evidence.append(
                "نموذج التعكير يقدّر قابلية مرتفعة (أمطار/ريح/طاقة موج) — تنبيه لا قياس."
            )
        ...
        fresh_jelly = [
            r
            for r in dataset.field_reports
            if r.kind == "jellyfish" and _report_freshness(r.reported_at, dataset.fetched_at)[0]
        ]
        if fresh_jelly and any(r.severity == "dense" for r in fresh_jelly):
            factors.append(
                TripRuinFactor(
                    ...
                    evidence_ar=[
                        "بلاغ ميداني حديث يؤكد وجود قناديل كثيفة في البقعة — تُفسد الخرجة."
                    ],
                    age_ar=None,
        ...
        fresh_debris = [
            r
            for r in dataset.field_reports
            if r.kind == "debris" and _report_freshness(r.reported_at, dataset.fetched_at)[0]
        ]
        if fresh_debris and any(r.severity == "dense" for r in fresh_debris):
            factors.append(
                TripRuinFactor(
                    ...
                    is_force_majeure=True,
                    evidence_ar=[
                        "بلاغ ميداني حديث يؤكد حطاماً كثيفاً (فضلات أودية) في البقعة — يُفسد الخرجة."
                    ],
                    age_ar=None,
    ...
    def _spring_neap(
        ...
        classification, ratio, reference = classify_spring_neap(target_range, list(ranges.values()))
        ...
        if reference is not None and reference < 0.10:
            ...
            notes.append("مدى مستوى البحر في النموذج ضعيف جداً؛ قد لا يلتقط المدّ الفلكي الحقيقي.")
        if gabes:
    ...
    def _species_axes(
        ...
        twilight_lead = lead is not None and _window_overlaps_twilight(
            lead, dataset.sunrise, dataset.sunset
        )
        night_lead = lead is not None and _window_overlaps_night(
            lead, dataset.sunrise, dataset.sunset
        )
        ...
        day_hours = [hour for hour in dataset.hours if hour.time.date() == dataset.target_date]
        day_wave_values = [
    ...
    def _species_activity(
        ...
        day_hours = [item for item in dataset.hours if item.time.date() == dataset.target_date]
        day_wave_values = [
        ...
        ordered_keys = [key for key in SPECIES_PROFILES if key != TargetSpecies.GENERAL] + [
            TargetSpecies.GENERAL
        ]
        illumination = moon_illumination(dataset.target_date)
        tide_delta = _tide_channel_delta(spring_neap.classification)
        lunar_day_delta = _lunar_day_delta(illumination)
        ...
        for species_key in ordered_keys:
            ...
            for hour in hourly:
                score, contributions = _species_hour_activity(
                    ...
                    lunar_day_delta=lunar_day_delta,
                )
                ...
                entries.append((activity, contributions))
            lunar_state = "قرب البدر أو المحاق" if lunar_day_delta else "خارج نافذة البدر/المحاق"
            basis = [
                *_activity_basis_ar(profile),
                f"المد اليوم من سلسلة مستوى البحر: {spring_neap.folk_label_ar} — أثر الاقتراب من الشاطئ {tide_delta:+d} (قوي +3 / ضعيف -3 / وسط 0).",
                f"اليوم القمري: {lunar_state} — قرينة شعبية غير معايرة {lunar_day_delta:+d}.",
                f"إضاءة القمر المحسوبة فلكياً لهذا اليوم: {illumination:.0%} — تُستعمل ليلاً فقط كقرينة ضوء (عتبة 70%).",
            ]
            activities.append(
                SpeciesActivity(
                    species=species_key.value,
                    ...
                    hourly=[activity for activity, _contributions in entries],
                )
            )
        return activities
    ...
    def _advanced_signals(
        ...
        """Reporting-only advanced signals (items 1-12 and 14-16; seasonal items
        13 and 17 were removed by user decision), all computed from real data or
        real astronomy - never invented and never fed into the binary decision
        or any safety gate.
        """
        signals: list[str] = []
        lat = dataset.location.latitude
        lon = dataset.location.longitude
        tz = dataset.fetched_at.tzinfo or UTC
        illumination = moon_illumination(dataset.target_date)

        # 1) Moon rise/set + illumination (upstream astronomy fields).
        if dataset.moonrise is not None and dataset.moonset is not None:
            signals.append(
                f"🌙 القمر اليوم: طلوع {dataset.moonrise.strftime('%H:%M')} · غروب {dataset.moonset.strftime('%H:%M')} · إضاءة {illumination:.0%} — من مزوّد الأرصاد."
            )
        else:
            signals.append(
                f"🌙 إضاءة القمر اليوم: {illumination:.0%} — حساب فلكي؛ أوقات الطلوع والغروب غير متوفرة من المزوّد."
            )

        # 2) Tide phase inside each recommended window (sea-level series).
        for window in recommended[:3]:
            window_hours = [h for h in hourly if window.start <= h.time < window.end]
            if not window_hours:
                continue
            rising = sum(1 for h in window_hours if h.derived.tide_state == TideState.RISING)
            falling = sum(1 for h in window_hours if h.derived.tide_state == TideState.FALLING)
            if rising > falling:
                phase = "المد صاعد (ماء يتحرك)"
            elif falling > rising:
                phase = "المد هابط (ماء يتحرك)"
            else:
                phase = "المد في سكون (حركة ضعيفة)"
            signals.append(
                f"🌊 نافذة {window.start.strftime('%H:%M')}-{window.end.strftime('%H:%M')}: {phase} — من سلسلة مستوى البحر."
            )

        # 3) Best timing intersection hour (real flags only, no solunar rule).
        best_hour: HourDecision | None = None
        best_score = -1
        for h in hourly:
            score = 0
            if h.derived.is_twilight:
                score += 2
            if h.derived.tide_state in {TideState.RISING, TideState.FALLING}:
                score += 1
            if (
                h.derived.pressure_change_3h_hpa is not None
                and h.derived.pressure_change_3h_hpa <= -1.0
            ):
                score += 1
            if score > best_score:
                best_score, best_hour = score, h
        if best_hour is not None and best_score >= 2:
            parts: list[str] = []
            if best_hour.derived.is_twilight:
                parts.append("غسق")
            if best_hour.derived.tide_state in {TideState.RISING, TideState.FALLING}:
                parts.append("مد متحرك")
            if (
                best_hour.derived.pressure_change_3h_hpa is not None
                and best_hour.derived.pressure_change_3h_hpa <= -1.0
            ):
                parts.append("هبوط ضغط")
            signals.append(
                f"⏱️ أقوى تقاطع ساعي: {best_hour.time.strftime('%H:%M')} ({' + '.join(parts)}) — جمع مؤشرات حقيقية."
            )

        # 4) Pressure 6h/12h trend (hPa series).
        pc6 = [
            h.derived.pressure_change_6h_hpa
            for h in hourly
            if h.derived.pressure_change_6h_hpa is not None
        ]
        pc12 = [
            h.derived.pressure_change_12h_hpa
            for h in hourly
            if h.derived.pressure_change_12h_hpa is not None
        ]
        if pc6 and pc12:
            signals.append(
                f"🫧 الضغط: تغيّر 6س من {min(pc6):+.1f} إلى {max(pc6):+.1f} hPa · 12س من {min(pc12):+.1f} إلى {max(pc12):+.1f} hPa — هبوط قوي = قرب جبهة. (عتبة الهبوط: ≤ -1 hPa خلال 3س)"
            )
        elif pc6:
            signals.append(
                f"🫧 الضغط: تغيّر 6س من {min(pc6):+.1f} إلى {max(pc6):+.1f} hPa — هبوط قوي = قرب جبهة. (عتبة الهبوط: ≤ -1 hPa خلال 3س)"
            )

        # 5) Offshore (toward-sea) current safety note.
        offshore = [
            h.derived.current_cross_shore_kmh
            for h in hourly
            if h.derived.current_cross_shore_kmh is not None
        ]
        if offshore:
            max_offshore = max(offshore)
            if max_offshore >= 0.4:
                signals.append(
                    f"⚠️ تيار نحو البحر يصل {max_offshore:.2f} كم/س (التيار النموذجي) — راقب سحب الخط نحو العمق. (عتبة التنبيه ≥ 0.4 كم/س)"
                )

        # 6) Sharp short waves (real steepness Hs/L₀).
        steepest = max(
            (h.derived.wave_steepness for h in hourly if h.derived.wave_steepness is not None),
            default=None,
        )
        if steepest is not None and steepest >= 0.03:
            signals.append(
                f"🌊 موج حاد قصير (انحدار {steepest:.3f}) — الرمي والتثبيت أصعب حتى لو الارتفاع صغير. (عتبة ≥ 0.03)"
            )

        # 6b) Weak surf (small + oblique/alongshore waves): a textual spot-quality
        # flag only - it changes no number and no decision (audit 2026-09-14).
        wave_heights = [
            h.forecast.wave_height_m for h in hourly if h.forecast.wave_height_m is not None
        ]
        incidences = {h.derived.wave_incidence for h in hourly}
        if (
            wave_heights
            and max(wave_heights) < 0.6
            and incidences & {WaveIncidence.OBLIQUE, WaveIncidence.ALONGSHORE}
        ):
            signals.append(
                "🏄 الكسرة ضعيفة (موج صغير ومائل) — تنبيه عرض فقط لا يغيّر الأرقام؛ رجّح القنوات العميقة القريبة من الكسرة أو غيّر البقعة."
            )

        # 6c) Cross sea (wind-wave vs swell arriving from clearly different
        # directions with comparable energy): a textual flag only. Both inputs
        # come from the model's own wave components; the angle is already shown
        # in §7 as "تشخيص بلا عتبة", and this line only translates it into a
        # field note (audit 2026-09-14, critical-review item 10).
        cross_pairs = [
        ...
        if cross_pairs:
            max_angle = max(angle for angle, _ in cross_pairs)
            max_share = max(share for _, share in cross_pairs)
            if max_angle >= 40.0 and max_share >= 0.25:
                signals.append(
                    f"🌊 بحر متقاطع محتمل: فرق زاوية موج الريح/السويل بلغ {max_angle:.0f}° "
                    f"وحصة النظام الأضعف {max_share * 100:.0f}% — تشخيص بلا عتبة قرار؛ "
                    "قد يقل استقرار الرصاص، والرمية الاختبارية أوضح دليل."
                )

        # 7) Sea-surface temperature versus the series mean.
        day_sst = [
        ...
        if day_sst and all_sst:
            mean_day = sum(day_sst) / len(day_sst)
            mean_all = sum(all_sst) / len(all_sst)
            diff = mean_day - mean_all
            if abs(diff) >= 0.5:
                signals.append(
                    f"🌡️ حرارة الماء {mean_day:.1f}° {'أدفأ' if diff > 0 else 'أبرد'} من متوسط السلسلة بـ {abs(diff):.1f}° — تغيّر حراري قد يحرّك السمك. (عتبة ≥ 0.5°)"
                )

        # 8) First rain after a dry spell (runoff → turbidity).
        day_start = datetime(
            dataset.target_date.year, dataset.target_date.month, dataset.target_date.day, tzinfo=tz
        )
        prior_48h = [
            item
            for item in dataset.hours
            if day_start - timedelta(hours=48) <= item.time < day_start
        ]
        prior_rain = sum(item.precipitation_mm or 0.0 for item in prior_48h)
        day_rain = sum(
            item.precipitation_mm or 0.0
            for item in dataset.hours
            if item.time.date() == dataset.target_date
        )
        if day_rain >= 0.5 and prior_rain < 0.5:
            signals.append(
                "🌧️ أول مطر بعد جفاف 48س — احتمال عكارة من الجرف أعلى؛ عاين الماء قبل الرمي. (عتبة: مطر اليوم ≥ 0.5مم وجفاف 48س قبله)"
            )

        # 9) True twilight windows (real solar geometry).
        nautical_start, nautical_end = solar_twilight_times(
            dataset.target_date, lat, lon, tz, -12.0
        )
        civil_start, civil_end = solar_twilight_times(dataset.target_date, lat, lon, tz, -6.0)
        if (
            nautical_start is not None
            and nautical_end is not None
            and civil_start is not None
            and civil_end is not None
        ):
            signals.append(
                f"🌅 الشفق الحقيقي: بحري {nautical_start.strftime('%H:%M')}-{nautical_end.strftime('%H:%M')} · مدني {civil_start.strftime('%H:%M')}-{civil_end.strftime('%H:%M')} — حساب فلكي."
            )

        # 10) Fastest sea-level change hour.
        rates = [
            (h.time, h.derived.sea_level_rate_m_per_h)
            for h in hourly
            if h.derived.sea_level_rate_m_per_h is not None
        ]
        if rates:
            peak_time, peak_rate = max(rates, key=lambda item: abs(item[1]))
            signals.append(
                f"🌊 أسرع تغيّر لمستوى البحر: {peak_rate:+.2f} م/س عند {peak_time.strftime('%H:%M')} — حركة الماء تبلغ ذروتها هناك."
            )

        # 11) Angler comfort (UV / apparent temp / wind).
        uv_values = [h.forecast.uv_index for h in hourly if h.forecast.uv_index is not None]
        temps = [
            h.forecast.apparent_temperature_c
            if h.forecast.apparent_temperature_c is not None
            else h.forecast.air_temperature_c
            for h in hourly
            if h.forecast.air_temperature_c is not None
        ]
        winds = [h.forecast.wind_speed_kmh for h in hourly if h.forecast.wind_speed_kmh is not None]
        gusts = [h.forecast.wind_gust_kmh for h in hourly if h.forecast.wind_gust_kmh is not None]
        if uv_values and temps and winds:
            max_uv = max(uv_values)
            mean_temp = sum(temps) / len(temps)
            mean_wind = sum(winds) / len(winds)
            comfort = (
                "مريحة"
                if max_uv <= 7 and 18.0 <= mean_temp <= 30.0 and mean_wind <= 25.0
                else "متعبة (شمس/حرارة/ريح قوية)"
            )
            gust_note = ""
            if gusts and max(gusts) >= mean_wind + 10:
                gust_note = (
                    f" · الهبات تصل {max(gusts):.0f} كم/س (قد تؤثر على التوازن رغم متوسط مريح)"
                )
            signals.append(
                f"🧍 راحة الصياد: {comfort} — أقصى UV {max_uv:.0f} · حرارة محسوسة {mean_temp:.0f}° · ريح متوسطة {mean_wind:.0f} كم/س{gust_note}. (حدود الراحة: UV ≤ 7 · حرارة 18-30° · ريح ≤ 25 كم/س)"
            )

        # 12) Model sources: one blended product (honest statement).
        signals.append(
            "🔎 نماذج الأرصاد تأتي مدمجة من مصدر واحد (مزيج Open-Meteo)؛ لا مقارنة بين نماذج مستقلة — ومقارنة رصد المحطة بالنموذج تظهر في الملخص."
        )

        # 14) Algal bloom from satellite chlorophyll (disclosed threshold).
        cwc = dataset.coastal_water_context
        if (
            cwc is not None
            and cwc.chlorophyll_a_mg_m3 is not None
            and cwc.chlorophyll_a_mg_m3 >= 3.0
        ):
            signals.append(
                f"🟢 الكلوروفيل {cwc.chlorophyll_a_mg_m3:.2f} mg/m³ — مرتفع؛ احتمال ازدهار طحالب، عاين لون الماء. (عتبة ≥ 3 mg/m³)"
            )

        # 15) Drifted leaves/debris after a recent swell then calm.
        max_wave_72h = max(
            (
                h.derived.max_wave_height_72h_m
                for h in hourly
                if h.derived.max_wave_height_72h_m is not None
            ),
            default=None,
        )
        today_wave = max(
        ...
        if (
            max_wave_72h is not None
            and today_wave is not None
            and max_wave_72h >= 1.5
            and today_wave <= 0.8
        ):
            signals.append(
                "🌿 أمواج قوية خلال 72س ثم هدوء — احتمال أوراق بوسيدونيا وحطام منجرف؛ افحص السنارة بعد رمية اختبارية. (عتبة: أمواج 72س ≥ 1.5م واليوم ≤ 0.8م)"
            )

        # 16) First trip after strong 72h waves.
        if max_wave_72h is not None and max_wave_72h >= 2.0:
            signals.append(
                "⚠️ أول خرجة بعد أمواج 72س قوية — عاين الشاطئ (تغيّر الكسرة والحواجز) قبل اختيار الموضع. (عتبة: أمواج 72س ≥ 2م)"
            )

        signals.insert(
            0,
            "كل ما يلي محسوب من داتا حقيقية (سلاسل النموذج، فلك) — عتبات التنبيه المذكورة اختيار إرشادي معلن، ولا تغيّر القرار أو السلامة.",
        )
        return signals
    ...
    def _overall_confidence(
        ...
        if recommended:
            ...
            weakest = min(covered, key=lambda item: item.confidence.score)
            breakdown = list(weakest.confidence.breakdown_ar)
            breakdown.append("المكونات تعكس أضعف ساعة داخل أفضل نافذة.")
        else:
            ...
            weakest = min(hourly, key=lambda item: item.confidence.score)
            breakdown = list(weakest.confidence.breakdown_ar)
            breakdown.append("المكونات تعكس أضعف ساعة خلال اليوم.")
        return ConfidenceResult(
            score=score,
            band=_confidence_band(score),
    ...
    def _factor_assessments(
        ...
        relation_counts: dict[WindRelation, int] = {}
        for hour in hourly:
            relation = hour.derived.wind_relation
            relation_counts[relation] = relation_counts.get(relation, 0) + 1
        if relation_counts:
            ranked = sorted(relation_counts.items(), key=lambda pair: -pair[1])
            if len(ranked) > 1 and ranked[0][1] == ranked[1][1]:
                tied = [
                    WIND_RELATION_AR[relation]
                    for relation, count in ranked
                    if count == ranked[0][1]
                ]
                dominant_relation_text = (
                    f"{' و'.join(tied)} (مناصفة {ranked[0][1]}/{len(hourly)} ساعة)"
                )
            else:
                dominant_relation_text = (
                    f"{WIND_RELATION_AR[ranked[0][0]]} ({ranked[0][1]}/{len(hourly)} ساعة)"
                )
        else:
            dominant_relation_text = "اتجاه الرياح غير معروف"
        ...
        if water_context is not None and water_context.availability == "available":
            ...
            age_text = (
                f"{int(water_context.age_hours + 0.5)}س"
        ...
        wind_mean_deg, wind_sample_count = dominant_wind_direction_deg(
            (h.forecast.wind_speed_kmh, h.forecast.wind_direction_deg) for h in hourly
        )
        if wind_mean_deg is None:
            wind_sector_text = f"اتجاه الريح {span('wind_direction_deg', '°', digits=0)}"
        else:
            qualified_bearings = [
                h.forecast.wind_direction_deg
                for h in hourly
                if h.forecast.wind_speed_kmh is not None
                and h.forecast.wind_direction_deg is not None
                and h.forecast.wind_speed_kmh >= MIN_DIRECTIONAL_WIND_KMH
            ]
            arc_start, arc_end = minimal_arc_extent_deg(qualified_bearings)
            start_sector = _wind_sector_ar(arc_start)
            end_sector = _wind_sector_ar(arc_end)
            if start_sector != end_sector:
                wind_sector_text = (
                    f"رياح متقلبة بين {start_sector} و{end_sector} "
                    f"(متوسط دائري {wind_mean_deg:.0f}° من {wind_sample_count} ساعة مقيمة)"
                )
            else:
                wind_sector_text = (
                    f"الريح السائدة: {_wind_sector_ar(wind_mean_deg)} "
                    f"(متوسط دائري {wind_mean_deg:.0f}° من {wind_sample_count} ساعة مقيمة)"
                )
        values: dict[str, str] = {
            ...
            "1.4": (
                f"{span('wind_speed_kmh', 'كم/س')}؛ التصنيف الغالب: {dominant_relation_text}؛ "
                f"إجهاد ريح تشخيصي {span('wind_stress_pa', 'Pa', derived=True, digits=4)}"
            ),
            "1.6": f"اتجاه البحر {orientation:.1f}°؛ الغالب: {dominant_relation_text}",
            "1.7": "تسلسل ساعي متاح؛ الغالب: " + dominant_relation_text,
            "1.8": span("pressure_msl_hpa", "hPa"),
            "1.9": pressure_drop_text,
            "1.10": pressure_change_text,
            "1.11": wind_sector_text,
...
def _wind_sector_ar(bearing_deg: float) -> str:
    """8-point Arabic compass sector for a meteorological bearing (from-direction)."""
    index = int(((bearing_deg + 22.5) % 360.0) // 45.0) % len(_WIND_SECTOR_LABELS_AR)
    return _WIND_SECTOR_LABELS_AR[index]
...
def _field_feasibility_limitations() -> list[str]:
    return [
        ...
        "قاعدة التنفيذ معلنة للتدقيق: تُجمع نقاط صعوبة التثبيت من أربع آليات (جرّ جانبي، حركة قاع، رجوع بحري، تيار مدّي)، والمستوى «متوسط» من 3 نقاط و«مرتفع» من 6، ويُخصم من 100: منخفض 0 · متوسط 15 · مرتفع 35 · غير معروف 25، مع خصومات أصغر للصوفة (12/30) والعكارة (10/25) والتيار الساحبي (8/20).",
    ]
...
def _confidence_breakdown(
    *,
    ...
    wave_inconsistent: bool,
    station_divergence: bool,
) -> list[str]:
    """Explain the confidence score as labelled components (presentation only).

    The score itself is unchanged; these lines only expose the arithmetic the
    engine already applies so a low number never looks arbitrary.
    """
    basis = round(
        completeness * 55.0
        + horizon_quality * 20.0
        + source_quality * 15.0
        + orientation_quality * 10.0
    )
    basis_line = (
        f"الأساس {basis}/100 قبل السقوف = اكتمال المتغيرات {completeness * 55.0:.0f} نقطة (وزن 55%) "
        f"+ أفق التوقع {horizon_quality * 20.0:.0f} (وزن 20%) "
        f"+ جودة المصادر {source_quality * 15.0:.0f} (وزن 15%) "
        f"+ اتجاه الشاطئ {orientation_quality * 10.0:.0f} (وزن 10%)."
    )
    caps: list[str] = []
    if critical_missing:
        caps.append(f"نقص متغير حرج للسلامة أو التنفيذ: {', '.join(critical_missing)}.")
    if wave_inconsistent:
        caps.append("اتجاه الموج غير متسق مع اتجاه البحر في البقعة.")
    if station_divergence:
        caps.append("اختلاف مهم بين رصد المحطة القريبة والنموذج عند وقت الجلب.")
    lines: list[str] = []
    if caps:
        lines.append(f"أبرز خافض للثقة: {caps[0]}")
        lines.append(basis_line)
        for extra in caps[1:]:
            lines.append(f"خافض إضافي: {extra}")
    else:
        # The only cap that can bind here is the free-model cap (85). Say so
        # explicitly instead of "no specific reducer": an 8-point reduction is a
        # specific, named reducer. If the basis is already at or below 85, the
        # cap does not bind and nothing is deducted.
        if basis > 85:
            lines.append(
                f"السقف الوحيد المطبَّق 85/100 = الأساس {basis} ناقص {basis - 85} نقطة "
                "لاعتماد النموذج المجاني بلا قياس مباشر في منطقة الكسرة."
            )
        else:
            lines.append("لا خافض مطبَّق؛ الأساس دون سقف النموذج المجاني (85) فلا يُخصم منه شيء.")
        lines.append(basis_line)
    return lines
...
def _report_freshness(reported_at: datetime, fetched_at: datetime) -> tuple[bool, str]:
...
def _worst_potential(values: Iterable[PotentialLevel]) -> PotentialLevel:
    ...
    # Note (audit 2026-09-14, item D1): UNKNOWN ranks above MODERATE so a single
    # data-gapped hour keeps the aggregated potential honest (Unknown) rather
    # than quietly reporting a known MODERATE — a deliberate conservative choice.
    order = {
        PotentialLevel.LOW: 0,
...
def _combine_holding_breakdowns(
    ...
    Carries the four-mechanism split over to the aggregated result so the
    window-level causes stay consistent with the hour-level causes.
    """
    ...
    dominant_key, dominant_level = max(mechanisms, key=lambda item: rank[item[1]])
    dominant = dominant_key if dominant_level != PotentialLevel.UNKNOWN else "none"
    representative = next((item for item in present if item.orbital_motion == orbital), present[0])
    dominant_labels = {
        "longshore_current": "الجرّ الجانبي",
        "orbital_motion": "الحركة المدارية قرب القاع",
        "return_flow": "الرجوع البحري قرب القاع",
