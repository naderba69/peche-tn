...
def test_minimal_arc_extent_deg() -> None:
    # single bearing
    assert minimal_arc_extent_deg([90.0]) == (90.0, 90.0)
    # empty
    assert minimal_arc_extent_deg([]) == (0.0, 0.0)
    # arc straddling north: 300→40 (not 40→300)
    start, end = minimal_arc_extent_deg([300.0, 320.0, 340.0, 0.0, 20.0, 40.0])
    assert start == 300.0 and end == 40.0
    # same-sector cluster straddling north stays within one sector
    start, end = minimal_arc_extent_deg([350.0, 355.0, 5.0, 10.0])
    assert start == 350.0 and end == 10.0
    # plain contiguous span
    assert minimal_arc_extent_deg([227.0, 250.0, 358.0]) == (227.0, 358.0)
...
def test_near_bottom_orbital_velocity_uses_representative_wave_height() -> None:
    # The input is Hs; the engine scales by Hrms = Hs/√2 so the diagnostic is not
    # inflated by the highest-third average (audit 2026-09-14, item B1).
    value = near_bottom_orbital_velocity_ms(1.0, 8.0, 3.0)
    assert value is not None
    expected = (
        math.pi
        * (1.0 / math.sqrt(2.0))
        / (8.0 * math.sinh(dispersion_wavenumber(2 * math.pi / 8.0, 3.0) * 3.0))
    )
