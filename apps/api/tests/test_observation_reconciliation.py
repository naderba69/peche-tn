...
def test_fresh_nearby_divergence_caps_confidence_and_marks_near_term_hours(
    ...
    observed_at = dataset.current_weather_observation.observed_at
    near_term = [
        hour for hour in result.hourly if abs((hour.time - observed_at).total_seconds()) <= 2 * 3600
    ]
    far_term = [
        hour for hour in result.hourly if abs((hour.time - observed_at).total_seconds()) > 2 * 3600
    ]
    # The divergence is a snapshot near fetch time: it caps only the hours close
    # to the observation (symmetric with _apply_observation_context), while hours
    # beyond ±2 h keep the model-only confidence.
    assert near_term
    assert all(hour.confidence.score <= 55 for hour in near_term)
    assert far_term
    assert all(hour.confidence.score > 55 for hour in far_term)
    direct_factors = [
        factor
