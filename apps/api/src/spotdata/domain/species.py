...
# ─────────────────────────────────────────────────────────────────────────── #
# Shared low-weight priors used by BOTH the decision opportunity score and the
# reporting-only activity indicator, so the same evidence never carries two
# different weights in two places (audit 2026-09-14, item C3).
# ─────────────────────────────────────────────────────────────────────────── #
SST_PREFERRED_DELTA = 6
SST_OUTSIDE_TOLERANCE_DELTA = -8
SHORE_MATCH_DELTA = 3
SHORE_MISMATCH_DELTA = -3
...
# Documented common surfcasting baits per target species. These are widely
# published practice (seabass favours moving/live bait in the surf zone, sparids
# take worms/crustaceans/small fish pieces), presented as a catalogue — never a
# catch guarantee (D10/D11).
BAIT_BY_SPECIES: dict[TargetSpecies, tuple[str, str]] = {
    TargetSpecies.GENERAL: (
        "حسب الموسم والنوع المتوفر (سردين، سبيط/حبار، دود بحري)",
        "أطعمة شائعة موثقة في الصيد بالقصبة؛ ليست ضمان مصيد.",
    ),
    TargetSpecies.EUROPEAN_SEABASS: (
        "طعم حي أو متحرك (سردين حي / سبيط) — القاروص يفضّل الحركة",
        "القاروص مفترس الكسرة ويميل للطعم المتحرك (Cervera et al. 2024؛ FishBase)؛ أطعمة شائعة لا ضمان مصيد.",
    ),
    TargetSpecies.GILTHEAD_SEABREAM: (
        "دود بحري، قشريات صغيرة، قطع سردين",
        "قاعي يتغذى على اللافقاريات والقشريات (FishBase؛ JMSE 2023)؛ أطعمة شائعة لا ضمان مصيد.",
    ),
    TargetSpecies.WHITE_SEABREAM: (
        "دود صخري / روبيان / قطع سردين",
        "يتغذى على اللافقاريات الصخرية والطحالب (FishBase Ref. 13780)؛ أطعمة شائعة لا ضمان مصيد.",
    ),
    TargetSpecies.STRIPED_SEABREAM: (
        "دود بحري / قشريات صغيرة",
        "قاعي رملي يتغذى على اللافقاريات القاعية (WoRMS/FishBase)؛ أطعمة شائعة لا ضمان مصيد.",
    ),
}
