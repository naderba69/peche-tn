...
import { clientOpenMeteoDisabled, fetchOpenMeteoBundle } from "@/lib/openMeteo";
...
function cacheKey(payload: ForecastDecisionRequest): string {
  return `peche-tn:v10:${JSON.stringify(payload)}`;
}
...
export async function requestDecision(
  ...
  // الهجين: المتصفح يجلب Open-Meteo أولاً بعنوان جهاز المستخدم، فإذا تعذر
  // (أو عُطّل) يبقى الحقل فارغاً ويجلب الخادم احتياطاً بطبقة الأمان الخادمية.
  let openMeteoData: ForecastDecisionRequest["open_meteo_data"] = null;
  if (!clientOpenMeteoDisabled()) {
    try {
      openMeteoData = await fetchOpenMeteoBundle(
  ...
  const response = await fetch("/api/v1/decisions/forecast", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(openMeteoData ? { ...payload, open_meteo_data: openMeteoData } : payload),
    signal,
  });
