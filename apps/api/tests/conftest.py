...
def build_dataset(
    *,
    ...
    return ForecastDataset(
        ...
        target_date=target_date if target_date is not None else TARGET,
        spot=SpotProfile(**spot_values),
        angler=AnglerProfile(**angler_values),
        hours=hours,
        sunrise=sunrise or datetime(2026, 9, 7, 6, 0, tzinfo=TZ),
        sunset=sunset or datetime(2026, 9, 7, 18, 30, tzinfo=TZ),
        fetched_at=fetched,
        sources=sources,
    )
